...
class ClientClass:
    ...
    def build_base(self):
        ...
        ssh("root@" + self.hypervisorIP, "pvesh create /nodes/" + hostname + "/qemu -vmid " + vid + " -name " + self.client.name + "-DB -memory 2048 -sockets 2 -cores 4 -net0 e1000,bridge=" + self.inter + " -net1 e1000,bridge=vmbr1 -virtio0=store:" + vid + "/ubuntu14-" + self.client.name + "-DB.qcow2")
        mkdir("/mnt/proxmox/images/" + vid)
        move("deploy/ubuntu_qemu/ubuntu14-" + self.client.name + "-DB.qcow2", "/mnt/proxmox/images/" + vid)
        ...
        vid = str(self.vid + 1)
        hostname = str(ssh("root@" + self.hypervisorIP, "cat /etc/hostname").rstrip())
        ssh("root@" + self.hypervisorIP, "pvesh create /nodes/" + hostname + "/qemu -vmid " + vid + " -name " + self.client.name + "-control -memory 2048 -sockets 2 -cores 4 -net0 e1000,bridge=vmbr0 -net1 e1000,bridge=" + self.inter + " -virtio0=store:" + vid + "/ubuntu14-" + self.client.name + "-control.qcow2")
        mkdir("/mnt/proxmox/images/" + vid)
        move("deploy/ubuntu_qemu/ubuntu14-" + self.client.name + "-control.qcow2", "/mnt/proxmox/images/" + vid)
        ssh("root@" + self.hypervisorIP, "pvesh create /nodes/" + hostname + "/qemu/" + vid + "/status/start")
        ...
        vid = str(self.vid + 2)
        hostname = str(ssh("root@" + self.hypervisorIP, "cat /etc/hostname").rstrip())
        ssh("root@" + self.hypervisorIP, "pvesh create /nodes/" + hostname + "/qemu -vmid " + vid + " -name " + self.client.name + "-worker -memory 2048 -sockets 2 -cores 4 -net0 e1000,bridge=" + self.inter + " -net1 e1000,bridge=vmbr1 -virtio0=store:" + vid + "/ubuntu14-" + self.client.name + "-worker.qcow2")
        mkdir("/mnt/proxmox/images/" + vid)
        move("deploy/ubuntu_qemu/ubuntu14-" + self.client.name + "-worker.qcow2", "/mnt/proxmox/images/" + vid)
        ssh("root@" + self.hypervisorIP, "pvesh create /nodes/" + hostname + "/qemu/" + vid + "/status/start")
        ...
        addWorker   = models.Nodes(client_id=self.client.id, type="worker", date_added=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), location="proxmox", IP="0.0.0.0", net=self.inter, vid=self.vid + 2)
    ...
    def build_worker_local(self):
        ...
        clientInterface = models.Nodes.query.order_by(models.Nodes.net.desc()).filter(models.Nodes.client_id == self.client.id).first()
        ...
        # Add to Database
        addWorker = models.Nodes(client_id=self.client.id, type="worker", date_added=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), location="proxmox", IP="0.0.0.0", net=self.inter, vid=vid)
        db.session.add(addWorker)
        db.session.commit() 
        return True
